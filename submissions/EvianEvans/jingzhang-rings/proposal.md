...
也就是说：这套机制**治理触发规则已定义，但测量规程尚未闭合**，本方案不宣称它当前即可运行。已定义的是触发条件（四项生态指标中至少两项连续低于基线，即触发"警报轮"复核）；**尚未定义**的是四项指标各自的观测单元、时空聚合口径、缺失数据处理规则、质量保证要求与跨年可比性控制——这五项须随数据序列的形成另行制定并公开。缺的既是观测值，也是尚未写定的观测规程；本方案如实标注这一点，而不是用"概念设计"四个字带过。
