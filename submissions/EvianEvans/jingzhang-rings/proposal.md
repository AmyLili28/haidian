...
2026 年 8 月获批的《京张铁路遗址公园沿线（人工智能创新街区重点地区）HD00-1601 等街区控规（2024—2035 年）》确立"一带一轴、两心多点"，其中**大钟寺为两心之一**——本方案的城市轮正落在这一官方锚点上；控规"以绿色公共空间引领区域更新"的路径与本方案"双轴夹一城、生态为硬约束"的结构同向。本方案是其概念深化层，不替代、不修改法定规划 `[source:SRC-HD00-1601-KONGGUI]`。
...
市政方面，管线、消防通道、防洪排涝与海绵城市条件均缺失（GAP-MUNICIPAL-001）。本方案不作管线迁改或消防通道的工程结论，只提两条体系性建议：其一，所有地面装置采用**浅基础**（桩基不深于 0.6 米）以避让管线；其二，滨水段设施采用**架空或贴装**，不缩小行洪断面。
...
| `alarm_ring_threshold` | 规则型 | known | 四项生态指标中至少两项同比下降 且 在试服务数上升 → 警报轮 |
...
两个比例指标的分子构成，此处逐项列明，与 `metrics.json` 中的 assumptions 完全一致：

| 指标 | 分子构成 | 性质 |
|---|---|---|
| `green_ratio` ≈ 0.2158 | 京张遗址公园绿轴、小月河滨水绿带、清河入河口生态缓冲带、三处年轮厅节点绿地 | 本方案提出或纳入维护的**新增设计量** |
| `public_space_ratio` ≈ 0.1054 | 双轴连续慢行主径、22 条东西缝合绿道、19 处里院广场、三处年轮厅前广场 | 本方案提出的**公共空间设计量** |
...
也就是说：这套机制**规则已定义，只待数据**，而不是"概念设计"四个字的托词。缺的是观测值，不是算法。

**据我们所知，这是首个把生态指标设为 AI 场景扩张硬约束的城区级机制**——技术扩张的配额不由预算或审批决定，而由一条河和两排树决定。它不新建任何大型构筑物：双轴一条已建成、一条在建，方案只叠加记录层。
...
3. 北京市人民政府门户网站：《京张铁路遗址公园沿线街区控规获批》（2026-08-12），HD00-1601 等街区控规"一带一轴、两心多点"，大钟寺为两心之一 `[source:SRC-HD00-1601-KONGGUI]`。
