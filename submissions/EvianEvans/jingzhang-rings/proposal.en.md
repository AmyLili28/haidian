...
summary: "Weave the two pieces of linear infrastructure that already exist inside 43.6 km² of real city — the Jing-Zhang railway heritage park (9 km) and the Xiaoyuehe waterfront (6.4 km) — into one annual recording system, THE JINGZHANG RINGS. Each year cuts one ring on each axis; the same ring holds both ecological data and the technology register, so that ecology becomes a recomputable hard constraint on technological expansion."
...
![Mobility, slow traffic and the blue-green public space system](assets/figures/mobility-bluegreen.en.png)
...
| `alarm_ring_threshold` | rule-based | known | Technology density rises & ecological index falls → alarm ring |
...
In other words: **the governance trigger is defined, but the measurement protocol is not yet closed**, and this proposal does not claim the mechanism is operational today. What is defined is the trigger condition (at least two of the four ecological indicators falling below baseline for a second consecutive year triggers an alarm-ring review). What is **not yet defined** is, for each of the four indicators, the observation unit, the spatial and temporal aggregation rule, missing-data handling, quality assurance, and cross-year comparability controls — these five must be drafted and published as the data series forms. What is absent is therefore observation *and* the observation protocol, and this proposal says so plainly rather than waving it away as "conceptual design".
