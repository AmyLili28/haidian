...
summary: "‘京张，再次开路’把本次 Agent 参与真实城市设计的开源征集视为一个正在展开、已有部分公开记录的制度原型，并把结构化任务、可追溯证据、公开纠错、保留给人类的终审与持续迭代转译为 AI 进入城市服务的可信机制；FP01 用一份未执行、可撤销的百日首用合同检验这套机制，不主张已获授权或实施。"
...
**从 Agent 参与真实城市设计，到 AI 进入真实城市服务：一次正在发生且已有公开记录的共创过程，一项仍待外部实证的城市实践**

一名不依赖智能手机、以轮椅通行为无障碍测试条件的早期职业阶段使用者，能否在一个有人值守的窗口完成一次学习或职业服务转介；AI 失效、含糊或被拒绝时，人工服务能否立即恢复？

*概念提案｜合同模板未执行｜不代表国家级试点认定、政府授权、采购、实施或既有成效*

## V0.14 评审导览与条件交付版：先判断，再深读

V0.14 不增加新的总体概念、场景或几何；它在重组既有证据的同时，新增逐门唯一决定责任、成本分类、维护、验收、条件释放、备选和退场结构，使方案成为一套可快速裁定、可交给专业团队继续深化的评审入口。推荐路径如下；它只导航可核查主张，不自行宣称任何维度得分。

| 阅读用时 | 先回答什么 | 建议证据路径 |
| --- | --- | --- |
| **30 秒：一个裁定** | 为什么必须是京张；AI 应以什么边界进入城市 | 本页主命题 → 总体空间图 → “一个人、一件事、一份可撤销合同” |
| **3 分钟：一条闭环** | 三区两翼、三重点区、五旗舰如何落到 FP01；失败后如何回到人工服务 | 总体空间（含三片区/五旗舰） → 双层验证 → FP01 三尺度 → 条件交付与退场 |
| **15 分钟：一条证据链** | 七项评分维度各自凭什么成立；哪些能现在判断，哪些必须继续 HOLD | 完整正文 → `metrics.json` 与三矩阵 → 三份 FP01 结构化证据及校验器 → 双语 HTML、A3/A0 与 `self_check.json` |
...
| AI 与城市规划创新性 / 15 | 六项城市转化能力、AI City API、可信应用公约、十二场景与空间原型共同构成城市级应用系统 | 六节点网络、七接口、五旗舰、三尺度原型、`geometry/public_space.geojson` |
| 可实施性 / 20 | 已把尺度、责任、数量、依赖、运营覆盖、成本、维护、验收、备选与退场绑定到同一条件交付页 | “V0.14 条件交付”、`visual/assets/fp01-delivery-control.json`、`assets/figures/technical-delivery-sheet.png`、实施与分期 |
...
### FP01 条件交付：现在能判断什么，仍不能放行什么

参赛者可控制的交付结构现已压缩为 **3 个重点区界面、3 个空间尺度和 6 项 D0 方法** [metric:key_area_interface_prototype_count] [metric:fp01_spatial_prototype_scale_count] [metric:fp01_d0_measurement_method_coverage_ratio]。它设置 **5 道 H0—H4 门、16 类交付角色，且每门只有 1 个决定责任角色** [metric:fp01_evidence_gate_count] [metric:fp01_delivery_role_class_count] [metric:fp01_hold_point_single_decision_owner_ratio]。
...
V0.14 继续采用“**京张总体空间 → 三处重点区域与五个旗舰 → FP01 一项任务 → H0—H4 实证门**”的评审顺序。首张总览图以官方“三区两翼”文字框架为上位依据，叠合临时 SITE/KEY 几何、京张公共证据脊、三处重点区域、五个概念旗舰和六项转化能力；它表达的是待官方边界校准的关系设计，不是新划法定分区、精确区位或确认项目 [metric:site_area_sqm] [metric:key_area_count] [metric:flagship_pilot_count]。

![京张总体空间、三区两翼、三处重点区域与五个概念旗舰](assets/figures/site-overview.png)

本版把“再次开路”明确为两个相连而不混同的证据层。**共创过程正在发生并已有部分公开记录**：公开任务、Agent 生成、来源与假设登记、机器校验，以及本方案谱系 V0.13 直接前序版的 intake 记录可以复核；正式评选、专业深化、最终判断和后续知识返还尚未观察，规则把这些判断保留给人类与专业团队。**本方案城市层仍待外部实证**：AI 进入真实服务之前，仍须取得场地、责任主体、人工基线、权利条件、专业审查、受控演练和另行授权。第一层目前证明部分人机专业协作可以被组织、追溯和修订，不替代第二层的 H0—H4 外部实证 [source:OPEN-CALL-PROJECT-STATEMENT] [source:AGENT-TASKBOOK] [source:SUBMISSION-INTAKE-PR-4017]。对应任务标准单列于本包的标准登记 [standard:PROJECT-AGENT-OPEN-CALL-TASKBOOK]。

![双层验证：正在发生且已有记录的公开共创过程与仍待外部实证的城市实践](assets/figures/double-demonstration.png)

图中的“已记录”只表示存在公开、可追溯的过程材料。`PR #4017` 只证明本方案谱系中的 V0.13 直接前序版已完成仓库 intake 并合并。本包不自行声明 V0.14 当前的 PR 或 intake 状态；若 V0.14 发布，只有对应实时 PR 与官方标签构成状态证据。创建或更新 PR 也只表示已提交仓库 intake，不证明 intake accepted，更不证明正式评选、采纳、实施、政府背书或国家级认定。机器账本 `visual/assets/agent-participation-ledger.json` 将这些事实等级和人类权限边界逐项锁定。

本版新增一份可机检、可由责任团队填写的 FP01 H0—H4 实证就绪附件，收录于 `report/narrative.md`。五道门的定义覆盖率为 1.0 [metric:fp01_evidence_gate_count] [metric:fp01_evidence_gate_definition_coverage_ratio]。
...
V0.14 继续把这两个零作为当前证据状态，并把专业团队下一步必须完成的工作做成一册概念交付控制簿：三片区各有一张可逆界面原型，六项 D0 指标都有事件边界、公式、缺失值和报告口径，H0—H4 每门都有角色类 RACI [metric:key_area_interface_prototype_count] [metric:fp01_d0_measurement_method_coverage_ratio] [metric:fp01_hold_point_raci_coverage_ratio]。
...
![FP01 一项候选服务任务的 1:500、1:100 与 1:50 多尺度空间原型](assets/figures/fp01-spatial-prototype.png)

空间原型把机制落到三个可继续深化的尺度：1:500 使用既有 `ROOM-TEST-001` 临时证据窗口检查到达、父公共空间与三种职责；1:100 使用 18×12 米可拆分模块检查双入口、人工核验、回执、申诉与轮椅回转；1:50 检查有人窗口、暂停状态、打印回执、无手机路径和退出恢复界面。比例与尺寸只用于设计测试，不是测绘、现状建筑、正式选址或获批工程；三尺度数量为 3，须在官方边界、现场测绘、权属和运营共创后重做 [metric:fp01_spatial_prototype_scale_count] [depth:three_key_area_detailed_design]。
...
1909 年，京张铁路证明中国能够自主勘测、设计、施工和管理一套复杂现代工程。今天的“再次开路”要把两种能力放在同一现场：AI 参与公共领域复杂专业工作的能力，以及城市机构审查、接管、纠错和暂停或终止具体 AI 服务的能力。第一次开路把工程能力写入地理；再次开路把智能能力与治理能力共同写入社会。它已经从方案生产端开始，但只有真实服务通过证据门后，才会从方法样本成为城市能力 [source:HIST-JINGZHANG-RAILWAY] [source:HIST-JINGZHANG-PARK]。
...
本方案以北京市规划和自然资源委员会海淀分局发布的《百年京张AI创新带城市设计国际方案征集资格预审公告》为第一依据，并以 `brief/site-package/` 中经维护者登记的任务、范围、枚举、指标和来源清单为机器可读依据。所有判断均拆分为可追溯来源、可复算指标、可校验图层和可人工复核假设。公告要求达到控制性详细规划的城市设计深度和规划综合实施方案的城市设计深度；V0.14 在不改动 V0.3 临时空间底盘的前提下，把既有 BUILD—TEST—LIVE 证据结构重新解释为六项转化能力及其稳定六节点网络。
...
这些文字重构没有改写 SITE/KEY 形状、3×4 用地拓扑或 V0.3 临时边界分母；嵌套街室按父公共空间 union 计数，公共空间面积仍为 432,600 ㎡ [metric:public_space_area_sqm]。由于官方精确边界、控制条件与独立专业签核尚未取得，完整 V0.14 文本仍是使用临时约束的开源概念建议，不是审批成果 [standard:MOHURD-ARCH-DESIGN-DEPTH-2016]。
...
V0.14 的“零坐标移动”不代表临时几何已被证实正确。仓库的临时边界依据已记录社区核查 #846 所指出的 OSM 公园参照与 SITE-001 之间约 412.5 米的偏离；社区核查 #1029 又指出，临时大钟寺重点区 PROV-KEY-003 的质心距大钟寺站约 2257 米、最近边界约 1788 米，而北京北站位于该临时面内。上述距离只是社区可复核的背景警报，不是官方纠偏、测绘结论或新边界。V0.14 因此把差异写入图层属性和假设账本，但保持全部坐标不动；“大钟寺站城与四象限步行连接”仍只表达待官方几何核准的关系目标 [source:SITE-PACKAGE] [source:COMMUNITY-GEOMETRY-CROSSCHECK-1029]。
...
本方案以 **SOURCE—STACK—PROVE—LIVE & MARKET—ENABLE—COMMONS** 为统筹研究网络：京张遗址公园构成历史与公共空间主轴，众智园承接 STACK 与 PROVE，北京 AI 原点社区承接 SOURCE、ENABLE 与 PROVE，大钟寺承接 LIVE & MARKET，两翼把专业服务、场景开放和公共经验送入 ENABLE 与 COMMONS。V0.14 沿既有七接口城市服务链保留可信运行实体，用嵌套房间和概念索引点保障公众行动，但不新增法定分区、新红线，也不改变 SITE/KEY 形状或 3×4 用地拓扑 [data:geometry/roads.geojson#ROAD-INTERFACE-001] [data:geometry/public_space.geojson#ROOM-BUILD-001] [depth:three_level_scope_framework]。
...
V0.14 以任务书提出的世界级 AI 创新生态为统筹研究对象，把高校院所、企业、算力算法数据要素、孵化与科技服务分别放入 SOURCE、STACK、ENABLE，再通过 PROVE、LIVE & MARKET 和 COMMONS 形成从技术到可复制 AI 应用能力的反馈网络。命名、视觉识别、总体空间结构、场景开放与长期运营共同服务“百年京张文化带、都市 AI 生活体验带、AI 融合创新带”；以上对照表已把三大定位、五大功能和三区两翼逐项连接到可见证据。
...
V0.14 沿用 V0.6 登记的七个可解析原型实体，每个实体关联一个主街室，并携带向三处街室复制完整接口套件的规则；实体数为 7，解析覆盖率为 1.0。保留这些机器实体是为了证据稳定，不代表旧协议命名仍是总体概念 [metric:protocol_interface_entity_count] [metric:protocol_interface_entity_coverage_ratio]。
...
V0.3 的可复算空间矩阵继续作为 V0.14 的证据底盘：南北方向为已登记的 **BUILD、TEST、LIVE** 三段，东西方向为 **知识研发、公共绿脊、公民接口、日常服务** 四类界面带，形成 12 个共享切割线的概念用地单元。V0.14 不把这组生命周期词继续当作总体品牌，而将 SOURCE、STACK、PROVE、LIVE & MARKET、ENABLE、COMMONS 叠加为跨片区网络。分类是功能建议而非现状或法定用地 [data:geometry/land_use.geojson#LU-BUILD-01] [depth:land_use_layout]。12 个 polygon 完整覆盖 SITE-001，实质 gap、outside 与 overlap 均为 0 [metric:land_use_coverage_ratio] [metric:land_use_overlap_area_sqm]。
...
| 重点片区 | 六节点网络角色 | V0.14 空间抓手 | 场景与实施门槛 | 证据引用 |
| --- | --- | --- | --- | --- |
...
V0.14 沿用从既有 persona 集合中登记的 3 个 `human_journey_anchor` 点 [metric:human_journey_count]。它们用于检验五个旗舰试点能否在普通人的服务旅程中安全运行，是连接场景、房间、接口和返还节点的概念审计索引，不是经测绘确认的无障碍路线、道路通行承诺或当前服务可用性证明 [depth:three_key_area_detailed_design]。
...
12 个概念功能面以共享切割线形成完整闭合分区；6 个小型建筑面只表达“既有调查后可适应性改造或轻量填充”的承载位置。由于没有现状建筑、权属、控规和工程条件，V0.14 不把任何对象提前判定为保留、改造、拆除或新建，而把逐栋测绘、产权核对、结构/消防评估、历史价值判断和功能适配作为进入设计深化的顺序门。
...
`visual/assets/fp01-evidence-readiness.json` 把每道门拆成**必需材料、责任角色、独立复核、停止动作和状态转换**；配套脚本拒绝把缺少日期、签署者、来源或复核的记录标成 `verified`。当前五道门统一保持 `pending_external_evidence`：H0 等待真实问题与人工基线，H1 等待测绘载体、实名主体、预算和采购路径，H2 等待无障碍、消防、市政、能源、交通、数据与安全等适用专业意见，H3 等待独立受控演练及失败/恢复记录，H4 等待法定授权、退出与交接。完整双语填报表收录于 `report/narrative.md`；它是证据获取协议，不是已完成的尽调或实施证明 [metric:fp01_evidence_verified_gate_count]。

本次征集的公开版本、Issue、PR 与机器校验，只能证明共创流程能够被追溯和修订；未来公开的人类判断记录也不得填写 FP01 的场地、使用者、基线、预算、专业审查、演练或授权字段，更不得把当前 `0/5` 外部实证改写为已通过。

### V0.14 条件交付页：把“怎么实施、怎么维护、怎么验收、怎么退出”放在一起

三处重点区不再只用功能名区分：众智园以公众旁观线、受控测试线和运营安全线形成 **24×12 米概念测试界面**；AI 原点以选择门、来源牌、有人台和回执坞形成 **18×12 米概念首用模块**；大钟寺以日常服务、争议补救和贡献轨形成 **30×6 米概念公共记录界面**。三组尺度只用于验证空间关系，现状尺寸、位置、通行与专业条件仍为空，必须由测绘和专业复核替换 [metric:key_area_interface_prototype_count]。
...
V0.14 为每道门另设唯一 `decision_owner_role` 与所需会签角色，共登记 16 类交付角色；真实组织、人员、任命、排班与签字仍为空。概念运营窗口必须同时覆盖**前线服务与无障碍、人工转介/拒绝与申诉权、运行安全与停止权**三类职责，独立评测不得由供应商自证代替。M03 的“两席位套件”只形成两席设计测试参照；现场容量取“实测无障碍席位、消防/安全核准容量、实际岗位覆盖”三者最低值，当前均不能据此宣布容量或吞吐 [metric:fp01_delivery_role_class_count] [metric:fp01_hold_point_single_decision_owner_ratio]。

![FP01 条件交付：尺度、责任、成本、验收、维护、备选与退出](assets/figures/technical-delivery-sheet.png)

单套设计测试包只登记 M01—M06 的最小测试数量。七类成本覆盖测绘现状、可逆构件、无障碍与有人服务、硬件连接、运营维护、数据/安全/独立评测/公共证据，以及拆除恢复与预备费；金额仍只按“经核实实际数量 × 清权可比单价 + 获批预备费”计算。当前实际数量、单价、金额、资金来源、采购路线和预算签署全部为 `null`，经核实成本输入为 0；缺报价意味着未知，绝不意味着零成本 [metric:fp01_design_test_boq_item_count] [metric:fp01_cost_plan_class_count] [metric:fp01_verified_cost_input_count]。

K01—K10 把可并行的 H0 问题确认与 H1 载体初筛，同后续专业核验、D0 基线、复演、决定、授权及交接串成不可跳级的关键依赖；它是交付控制模型，不是已承诺工期 [metric:fp01_critical_dependency_step_count]。AT01—AT12 把验收分成 8 项方案结构审查与 4 项现场/授权审查；REL00—REL05 的六级释放全部保持 `external_release_granted=false`。MNT01—MNT04 分别规定每次开放前、重大版本变化后、事件发生时和退出/换运营方时的维护与恢复动作；ALT01—ALT04 则为载体、数据、独立评测、采购/授权失败准备纯人工、可移动、非公众复核或完整关闭方案 [metric:fp01_acceptance_indicator_count] [metric:fp01_release_stage_count] [metric:fp01_fallback_alternative_count]。
...
“返还”不是把城市数据或公共权力让渡给技术提供者，而是在期限届满、重大版本变化、证据不达标、申诉久未解决、运营方退出，或权利/数据/工程条件失效时，把服务能力安全地交还给公众与城市运营体系。V0.14 沿用已登记的 BUILD、TEST、LIVE 三个能力返还节点，数量为 3 [metric:capacity_return_node_count]；它们分别承担开放方法与失败证据、无应用服务与可达性修复、申诉补救与公共记忆返还 [data:geometry/public_space.geojson#RETURN-BUILD-001] [data:geometry/public_space.geojson#RETURN-TEST-001] [data:geometry/public_space.geojson#RETURN-LIVE-001]。12 个要求回程票的应用场景均可反向解析到相应返还节点，引用覆盖率为 1.0；这证明机器引用闭合，不证明能力已经交付 [metric:return_ticket_reference_coverage_ratio]。三个节点均为 `concept_only` 点位索引，不是既有机构、运营主体或建设承诺。
...
长期运营的核心不是重复举办科技节，而是让六节点网络持续学习并留下可复用的 AI 应用能力。V0.14 建议形成“季度旗舰试点开放日—半年证据审查—年度京张开路大会—持续版本日志”的节奏：开发者和运营方提交测试与市场证据，居民与用户提出问题或申诉，专业团队评估空间、技术、商业、法律与伦理影响，再公开说明某项试点为何进入下一门、扩大、修改、暂停或终止。年度大会可集中展示全球案例、开源工具、首用实践、失败复盘、贡献荣誉与可信运行规则的下一版草案；这是一项运营概念，不代表主办方已确定活动、预算或政策安排 [source:AGENT-TASKBOOK] [depth:phasing_implementation]。

## 指标体系、面积复算与合规矩阵

V0.14 把指标分为三类：可由 GeoJSON 在 EPSG:4548 复算的设计指标；来自公告的近似范围数字；以及因缺少官方条件而保持 `unknown` 的法定/工程指标。任何设计值都不能升级为审定指标，任何 unknown 都不能用 0 或“平均值”填补 [depth:metrics_recalculation]。
...
V0.14 进一步把 H0—H4 从概念停止点升级为独立的实证登记簿：门数为 5，门定义覆盖率为 1.0 [metric:fp01_evidence_gate_count] [metric:fp01_evidence_gate_definition_coverage_ratio]。
...
V0.14 条件交接另登记 16 类交付角色、每门 1 个决定责任角色，以及 12 项验收指标，其中 8 项可在方案层判断、4 项依赖现场或授权证据 [metric:fp01_delivery_role_class_count] [metric:fp01_hold_point_single_decision_owner_ratio] [metric:fp01_acceptance_indicator_count]。
