...
<!-- LEDGER:BEGIN -->
它由 `analysis/build_rights_ledger.py` 从 `manifest.json` 生成，而不是手写的资产分组清单。差别是实质性的：手写的台账枚举「作者记得的资产」，生成的台账枚举「实际随包的文件」，任一文件没有权利归类即**构建失败**。当前 **106 个文件全部归类**：

| 清权类别 | 文件数 | 含义 |
|---|---|---|
| `author-originated` | 84 | 本方案原创，无第三方内容 |
