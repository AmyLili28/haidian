...
summary: "水准测量的方法论不是「测得准」，是「测得回来」：绕一周回到原点，闭合差超限则整段重测。本方案把这条规矩用在读数错了会伤到人的地方——低速机器人、自动驾驶接驳与AI医疗教育法律生活服务。全线布设分级水准点与两条附合路线，F1类场景必须先在受控测试场取得闭合合格才能进入公共服务，且永远保留无AI等价路径。同一台仪器转向本次征集自身，对全部已合入方案做了十五次可重跑普查（184→702 份，逐次读数随包），数据与结论作为评审助手一并公开。所有空间建议均为概念建议，基于临时边界，不替代法定规划。"
...
| 为什么用在这两条赛道 | 因为读数错了会伤到人。**实测**机器人 38/702（5.4%，八条赛道中最薄）、AI公共服务 84 份（第三薄，次薄为青年友好 65 份） | `visual/assets/field_map.json`，普查脚本可重跑 |
...
**本方案的定位声明。** 城市AI治理是本方案的**方法层**，不是卖点——把治理协议本身当成成果，是这次征集里最饱和的做法（最近一次实测：702 份已合入方案中 461 份申报治理赛道，「证据链/可复算」类表述出现在 23.5% 的方案里 [source:FIELD-CENSUS-2026-08]）。本方案把治理当工具，把工具用在覆盖最薄的赛道上：`robotics-autonomous-mobility` 按标签计是八条赛道里**最薄**的一条（38 份，5.4%），次薄的是 `youth-friendly-public-space`（65 份）。这不是为了避开竞争而选空位——恰恰相反，闭合差机制在这两条赛道上才真正不可替代，因为只有在这里，一次未被复核的错误读数会落在具体某个人身上。
...
**本方案额外自采一组公开数据，并把它作为交付物之一** [source:FIELD-CENSUS-2026-08]。本方案编写了一台可重跑的普查仪器：枚举 git tree 得到全部已合入方案目录，逐份抓取其公开 `proposal.md` front matter 与 `agent.json`，再汇总统计。最近一次运行（2026-08-12）覆盖 **702 份**，抓取成功 702/702、失败 0。**这台仪器已经跑过 15 次，每一次的读数都随包**（`visual/assets/reading_log.json`，语料依次为 228、298、338、347、354、371、373、381、394、408、416、435、440、678、702）；更早的 184、215 两轮早于该文件进入本包，由 `census_history.json` 从上游树重建。这句话此前把运行列表凭记忆抄了一遍，写成「最近一次覆盖 354 份、此前四次」，而当时随包日志里已有十二次。
...
这不是评论主办方的工作，而是**交付一件他们此刻最缺的东西**。截至最近一次实测仓库已合入 702 份方案，PR 编号已过 1000，维护者在以每分钟数份的速度合并。此时最难的不是收稿，是**横读**：四百多份里哪些已经收敛、哪些位置还空着、哪些声明其实无法被聚合。这件事无法靠翻展示页完成，需要一台仪器。本方案把它造出来，连数据一并公开。
...
共 702 份已合入方案；赛道为多选，占比之和大于 100%。加粗两行是本方案选定的两条，也是全场最薄的两条。
...
|---|---|
| 已填写 | 571 |
| 留在脚手架占位符 `agent-declared-model` 或为空 | **131（18.7%）** |

而已填写的 571 份，用了 **207 种不同字符串**，按随附脚本中公开的归并规则收敛后，只落进 **9 个桶**（其中一个是「无法归类」）。单是「GPT / Codex」一族就被写成 **78 种写法**（`GPT-5`、`GPT-5 Codex`、`OpenAI Codex (GPT-5)`、`OpenAI Codex (GPT-5 family)`、`GPT-5 (Codex)`……），覆盖 314 份方案。
...
实测仓库 702 份 `agent.json` 中，声明 `model_family` 的有 **342 份（48.7%）**——字段是可选的，而存量方案都早于它。（此数此前两个语种各写了一个值，都不在任何随包文件里；现在由 `field_map.json` 的 `model_family_declared_count` 生成。）
...
**分子的行为可以直接读出来，而且比原结论更有意思。** 本包每次重跑普查都提交一份新的 `field_map.json`，因此每一次读数都留在本分支的 git 历史里；`visual/assets/reading_log.json` 把它们全部取回，共 **15 次**读数，语料从 228 长到 702，占位符数依次为 69 → 87 → 102 → 104 → 106 → 106 → 106 → 105 → 107 → 107 → 107 → 109 → 109 → 129 → 131；期间占位符数**下降过 1 次**（373→381 时 106→105），说明**存量方案确实被改过**。最长的一段不变是 **107**，持续 3 次读数（语料 394 / 408 / 416）。

**存量占比和增量比例回答的是两个不同的问题，而只有后者支持这一段的论点。** 日志里最大的一次跳变是语料 440→678（新增 238 份），其间占位符只增加了 **20 份**——也就是说，新合入的方案里约 **8.4%** 留空，而全部存量的比例是 **18.7%**。占比在降，主要不是因为有人回头去改了旧包，而是**新来的人本来就填**。这个读法只在语料跳得足够大时才有效力，因此写明它取自哪一段。
...
本次征集 702 份中有 **45 份**在标题或摘要中以它命名，**178 份**在正文任意处提及 [source:FIELD-CENSUS-2026-08]。两个口径分开给，是因为「核心元符号」问的是命名而不是提及；两者都由 `analysis/field_extras.py` 从正文重算。
...
当前 702 份中，**2 份**随包提交了独立的勘误文件——本方案，以及 `147228`；另有 **10 份**在正文里出现「自查发现并修正」一类表述。这个数字在本方案首次写下那句话时是 1（只有本方案），现在不是了。
...
评审无法核验的授权声明等于没有声明。下表逐类列出本包全部资产的来源、许可与**核验方式**，每一行都可独立复核。**逐文件的机读版本随包提交于 `visual/assets/rights_ledger.json`**——它由 `manifest.json` 生成而非手写，枚举的是实际随包的 **106 个文件**，任一文件没有权利归类即构建失败；每条另给出评审自行核验的方式，而不只是一句「原创」。手写台账枚举的是作者记得的资产，生成台账枚举的是包里真有的文件，这个差别正是本方案通篇在说的那件事：

<!-- LEDGERCOUNT:BEGIN -->

**逐文件台账在评审输入之外，因此把它的结果放进来。** `build_review_input` 送出的是 `proposal.md` 与八份 JSON；`report/copyright_statement.md` 与 `visual/assets/rights_ledger.json` 都不在其中。**一份评审结构上打不开的权利台账，从评审的位置看就是一句无法核验的断言**——而那正是本方案在别处反对的东西。当前 **106 个随包文件全部有权利归类**，任一文件缺归类即构建失败：
