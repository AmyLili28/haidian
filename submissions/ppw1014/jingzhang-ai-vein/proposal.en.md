...
Scenarios are organised on a three-step lighting ladder [metric:scenario_lighting_e0_count] [metric:scenario_lighting_e1_count] [metric:scenario_lighting_e2_count]: **E0 concept** (4 nodes, unlit, direction only), **E1 pilot-ready candidate** (5 nodes, lighting preconditions met — still not an operating approval), **E2 pilot-validation design** (3 TVS, design depth prepared for pilot validation; field-validation track record: zero — real operation requires separate permits and safety assessment). The ladder is an evidence gate, not a schedule promise.
