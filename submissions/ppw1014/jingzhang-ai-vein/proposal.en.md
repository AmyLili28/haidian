...
summary: "Using the Jing-Zhang Heritage Park as the historical and public-space spine, with the Zhongzhiyuan, Beijing AI Origin Community, and Dazhongsi key areas as innovation anchors and the Zhongguancun Technology Services Wing and Xiaoyue River Scenario Enablement Wing as coordinating wings, the proposal builds an AI city framework: one belt, three cores, multi-point scenarios, and a blue-green slow-mobility composite ring; the twelve AI scenario nodes are managed at three maturity levels (E0 concept, E1 pilot candidate, TVS pilot design) and none is an approved operation."
...
![Naming system and visual identity (VI) direction](assets/figures/brand-vi.en.png)
...
Scenarios are organised on a three-step lighting ladder [metric:scenario_lighting_e0_count] [metric:scenario_lighting_e1_count] [metric:scenario_lighting_e2_count]: **E0 concept** (4 nodes, unlit, direction only), **E1 pilot-ready candidate** (5 nodes, lighting preconditions met — still not an operating approval), **E2 pilot-validated** (3 TVS, pilot-validation-grade design; real operation requires separate permits and safety assessment). The ladder is an evidence gate, not a schedule promise.
...
- Building footprints are indicative design objects — 158 footprints, ~33.2 ha, four clusters [data:geometry/buildings.geojson#BLDG-001] [metric:building_footprint_area_sqm] [metric:building_count] — not an existing-building inventory [depth:existing_conditions_diagnosis].
