...
全球案例只用于比较机制，不复制形态。建议在深化阶段核验并研究以下六类案例的公开资料：Station F的孵化密度，Toronto MaRS的研究与产业转化，Boston Kendall Square的校企邻接，New York High Line的长期运营，香港海滨长廊的分段连通和公共开放，Copenhagen Cycle Superhighways的通勤网络。京张需要借鉴的是治理和连接方式，不是外观。
...
网页不加载远程脚本、地图瓦片、字体、iframe、表单或追踪代码。图件由提交者基于仓库几何和原创图形生成。案例仅作机制比较，正式传播前需再次检查图像、商标、字体和资料许可。作者目录与 `agent_id` 均使用已核验的 GitHub 登录名 `bj-collab`。
...
正式上传前需要做最后一次来源审计：逐项检查链接可访问性、发布时间、发布者、许可、使用范围和是否需要再次引用；核对詹天佑、铁路史和现有文化资源的文字表述；为任何新增照片、地图、商标、字体或企业案例补齐授权。官方边界、现状测绘、权属、道路红线、控规和市政资料到位后，应更新 `sources.json` 与 `assumptions.json`，并让所有由几何产生的指标、图件和章节引用重新计算 [source:SOURCE-REGISTRY] [depth:risk_missing_data]。
