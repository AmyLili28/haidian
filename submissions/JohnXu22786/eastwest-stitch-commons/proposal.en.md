---
title: "STITCH LIVING ROOM: Jingzhang Heritage Park AI Commons & East-West Stitching (Concept)"
...
summary: "A conceptual urban design for the Centennial Jingzhang AI Innovation Belt. Stitch Living Room (STITCH·JZ) uses the Jingzhang Railway heritage green belt as a stitching interface to organize functional complementarity and public connectivity between the east and west streets, translating the historically severed halves into a crossable, stayable, and conversable urban living room. 'One Belt, Three Nodes' covers Stitch Bridge Court, East-West Market Gallery, and Stitch Axis. An 8-factor mechanism (Land, Space, Industry, Capital, Talent, Compute, Data, Scenarios) is strictly mapped to the official Two Wings (Zhongguancun Technology Service Wing & Xiaoyuehe Scenario Empowerment Wing) and Three Key Areas without fabricating corporate or financial commitments. Twelve AI+ scenario cards adhere to self-imposed design governance rules (anonymous aggregation only, human fallback, appeal & shut-off), strictly rejecting biometric surveillance. Phased implementation and annual event brand system close the loop. All content is conceptual advice on a provisional boundary; recalculate upon official data release."
...
![Site Overview (provisional geometry)](assets/figures/site-overview.en.png)
...
Twelve AI+ scenario cards strictly adhere to self-imposed "anonymous aggregation + human review" governance rules; the five representative cards below are complemented by the complete 12-card set in “Review-Closure Addendum”. Biometric surveillance is prohibited:
...
![AI Innovation Ecosystem (8-Factor Architecture)](assets/figures/ai-innovation-ecosystem.en.png)
...
![Key Areas & Three Node Detailed Design Index](assets/figures/key-areas.en.png)
...
![Mobility & Blue-Green Space Analysis](assets/figures/mobility-bluegreen.en.png)
...
### Review-closure implementation matrix and regional loops
The formal Two Wings are only “Zhongguancun Technology Service Wing” and “Xiaoyuehe Scenario Empowerment Wing”. “Higher-education intelligence interface” and “international living interface” remain local conceptual interfaces and never replace the taskbook wings. The three positions map to Jingzhang heritage, urban AI living experience, and AI-integrated innovation; the five functions are carried by heritage narrative, scenario cards, the eight-factor ecosystem, public services, and transparent governance. Beiwèi Community, Future Science City, Huairou Science City, E-Town and Jing-Jin-Ji use resource–interface owner–entry condition–feedback loops; owners are stated as proposed local/park/research contacts pending confirmation, with no fabricated cooperation.

| Regional interface | Resource | Proposed interface owner | Entry condition | Feedback |
|---|---|---|---|---|
|Beiwèi Community|resident issues, volunteers|local community/operator TBD|co-creation, privacy review|monthly issue log|
|Future Science City|research/test topics|park or research body TBD|public topic, safety gate|quarterly cards|
|Huairou Science City|science content|research-park body TBD|content licence|annual exhibit review|
|E-Town|industry problems|industrial park body TBD|voluntary firms, minimum data|test review|
|Jing-Jin-Ji|heritage/transport lessons|public liaison channels|public case, rights check|annual update|

### Eight-factor ecosystem and carrier boundaries
Land, space, industry, capital, talent, compute, data and scenarios are eight parallel factors. Zhongzhiyuan receives talent/compute/industry questions and outputs testable cards; AI Origin Community receives public-life needs and outputs anonymous feedback; the Technology Service Wing handles resource and transfer interfaces; the Xiaoyuehe Wing handles scenario opening, feedback, appeal and shut-off. None implies a partner, funding, equipment or approval commitment. The eight-factor table and the 12 scenario cards use the same input/output/owner/boundary vocabulary.

### Twelve AI scenario cards and T1–T3 protocols
Each card records object, node, problem, input, AI function, output, human review, privacy, operator, failure/shut-off and metric. SC01 commuter/Axis/delay/anonymous flow→demand forecast→signal advice→traffic review→no tracks→operator TBD→shut off on false alarms→waiting percentile; SC02 wheelchair/Bridge Court/broken route/ramp status→route rules→alternative route→manual escort→no biometrics→service desk→human fallback→continuous access; SC03 blind visitor/three nodes/illegible information/text→voice guide→paper+voice→human read-through→no voice retention→wayfinding operator→paper fallback→completion; SC04 elder/Market Gallery/insufficient rest/anonymous dwell→need prompt→manual placement→community review→aggregate only→community operator→remove false prompt→rest access; SC05 family/Market Gallery/conflicting events/time windows→schedule advice→time notice→event lead→no child ID→event operator→shut off if crowded→complaints; SC06 trader/Market Gallery/uneven rotation/booking+asset state→rotation advice→public roster→assembly approval→no business secrets→market operator→rollback→fairness; SC07 maintainer/three nodes/faults/anonymous facility image→state detection→work order→human confirm→people removed→maintenance operator→close false alarm→repair time; SC08 visitor/Time Gallery/source search/licensed metadata→summary→source card→curator→no portraits→exhibit operator→do not publish missing source→citation completeness; SC09 researcher/Bridge Court/unclear entry/public topics→matching→entry checklist→ethics/safety gate→minimum data→service interface→reject if gate fails→pass rate; SC10 night resident/Axis/light comfort/environment sensor→comfort prompt→lighting advice→staff review→no tracking→maintenance→fixed-light fallback→appeals; SC11 international family/Market Gallery/language gap/text→translation draft→bilingual sign→language review→no profiling→wayfinding→replace errors→readability; SC12 developer/three nodes/model drift/de-identified samples+logs→drift detection→version report→evaluation panel freeze→no personal data→developer community TBD→shut off over threshold→group error gap.

T1 uses two weeks of manual counts as baseline and groups time/weather/wheelchair; traffic professionals and community assembly approve thresholds, then freeze the version and output percentiles, confusion table and review logs weekly. T2 uses manual inspection gold samples grouped by day/night, wet/dry and asset type; maintenance and safety reviewers approve thresholds, freeze the version, output work-order evidence biweekly. T3 uses a human language-checked set grouped by language, age/visitor and screen/paper; language reviewer and community representatives approve thresholds, freeze the version, output differences and appeals monthly. There are no measured results yet; unexplained or unsafe results trigger human takeover, rollback or shut-off.

### Spatial and operational closure
The three AI landmarks are “Stitch Bridge Court”, “Jingzhang Time Gallery” and “Xiaoyuehe Data Garden”. The honours wall displays only verified public facts; the component library includes wayfinding, reversible seats, rotating stalls, human desk, privacy sign, paper map box, child-height sign and detachable display rack. The cultural narrative is Jingzhang heritage → Zhongguancun innovation culture → explainable AI culture. Developer community, open scenes, international outreach and talent/company/developer conversion require public topics, licences, safety gates and handover. The responsibility matrix and rights ledger are conceptual controls, not completed collaboration evidence.

### Rights and metric boundary
“Stitch Living Room/STITCH·JZ/One Belt Three Nodes” and the logo are internal working names; trademark, prior-rights and commercial font checks are unfinished, so registration, commercial use and external licensing are prohibited before clearance. Article 39 of the Barrier-Free Environment Law is limited to listed public-service settings; the Generative AI Measures do not automatically cover non-generative sensing. Human fallback, anonymous aggregation, appeal and shut-off are self-imposed design governance rules. Every metric ratio carries source, formula, confidence, intended-use limit and official-data recomputation trigger in `metrics.json`.
