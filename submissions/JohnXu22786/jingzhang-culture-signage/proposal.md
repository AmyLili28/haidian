...
summary: "沿京张铁路原线绿带与遗址公园组织一体化文化叙事导视标识系统概念：「章痕·京张叙事导视带」以「一带三站」——站牌叙事站、道钉符号步道、信号声音亭——转译铁路历史、站点故事与创新带当代叙事为可识别、可阅读、可更新的标识体系与慢行体验；ZHANGHEN · JINGZHANG NARRATIVE WAYFINDING BELT 为固定英文主品牌，WAYMARK·JZ 仅作内部工作代号；12张AI+场景卡覆盖文化、公共服务、空间、治理、产业、交通六域，仅匿名聚合、关键决策人工复核、禁止过度监控，每个AI场景均有非AI降级路径；配套统一品牌与Logo/VI、生态图谱、产业测试场景、荣誉体系、组件库、开发者社区、场景开放与招引转化机制、年度活动体系与近期试点执行矩阵。全部内容为概念建议与参考方案，基于 provisional 边界，官方数据发布后复算。"
...
**agent.4 空间—场景—运营核对映射（概念、provisional，可供专业团队深化研究）**：东西缝合采用「轨道遗产缝合廊」空间类型，连接对象为京张遗产线索、绿带慢行主轴、众智园与北京AI原点社区的公共入口；公共体验路径为“站牌叙事站阅读 → 道钉符号步道行走 → 社区/园区公共入口停留”。南北贯通采用「跨街慢行门廊」空间类型，连接对象为三重点区、既有轨道/公交出入口、社区服务节点与大钟寺站周边公共空间；公共体验路径为“出站即入线 → 贯通三站 → 进入大钟寺产业聚集区”。这些是可拆卸标识、地面符号、声音/文字服务和预约讲解的概念连接对象，不是桥隧、道路红线或轨道工程方案。

三区落位与运营建议逐项对应：众智园AI自主创新加速区以「道钉符号步道」为创新测试与参访叙事入口，建议运营方为园区公共空间运营团队，联合高校评测；北京AI原点社区以「站牌叙事站」为居民、青年与访客的阅读/共创入口，建议运营方为社区议事与志愿者联盟；大钟寺AI产业聚集区以「信号声音亭」和可转换的站前公共空间为智能原生消费与商务场景入口，建议运营方为园区运营团队与商户共治小组，承接多语导览、预约参访、内容更新和维护供给测试。三条路径均保留人工讲解、纸质/触觉站牌、静音模式和现场值守兜底；点位、权属、文保、噪声与交通许可均待专业核验。
...
|---|---|---|---|---|---|---|---|---|
| 「站牌叙事站」试验亭 | 区政府牵头；高校、运营方、志愿者协作 | 场地与文保审查（概念） | 低 | 仅匿名聚合使用量；端侧处理 | 日常开放＋人工值守班次 | T1：30天内史实核验批次完成率=100%；4周试点周均使用相对基线提升≥10% | 意见箱＋线上评议；年度公示 | T1任一阈值未达或出现P0风险→撤收/改临展并复盘 |
| 「道钉径」试验段 | 区级交通与规自部门牵头；街道、社区协作 | 占道与慢行审批（概念） | 低 | 地面符号非感知性，无采集 | 8周每周巡检＋节事前后专项 | T2：完好率≥95%；参考行程计时准确率≥90%，均按协议样本与公式计算 | 沿线议事会＋问卷 | 连续2周任一阈值未达→停用该段，重测后再议 |
| 「信号亭」无障碍声景亭 | 残联与文旅部门牵头；无障碍专家、志愿者协作 | 噪声与设施审批（概念） | 中 | 音频本地缓存；无个人档案 | 音量分级＋静音时段；离线可用 | T3：盲文/触觉完好率≥95%；4语覆盖；有效交互投诉率≤5% | 体验测试（含视听障碍用户） | T3任一阈值未达、投诉率>5%或出现安全事件→降级为文字站牌 |
| 巡检＋密度集成试点 | 运营主体牵头；辖区部门、商户协作 | 设备与数据备案（概念） | 中 | 仅聚合热力；明确保留与访问权限 | 巡检闭环≤24小时；人工复核全覆盖 | T3运行子项：24小时闭环率≥90%；误报率≤5%，按≥50条工单样本计算 | 月度公示＋公开审计 | 出现重识别风险或任一连续2周低于阈值→立即停用并下架 |

上述 T1–T3 均为参与者提议、provisional、非官方验收标准；完整 baseline、sample、window、formula、numeric threshold、owner、状态与失败退出记录见 `visual/assets/test_protocols.json`，正式试点启动前须由参与主体和专业团队共同核定并公示。

### 产业测试与 go/no-go 协议（参与者提议，provisional，非官方）

- **T1 参访转化（众智园AI自主创新加速区／中关村科技服务翼）**：baseline 为试点前连续4周、同一开放时段的匿名预约台账；sample 为 baseline 与 pilot 各不少于30个合格预约团组；window 为各4周并按星期配对；formula 为 `completed_visits / eligible_bookings`，`lift=(pilot_rate-baseline_rate)/baseline_rate`，baseline=0 时改报绝对差并不做相对 lift；numeric threshold 为 `lift≥10%` 且两期 sample 均≥30；owner 为园区公共空间运营方牵头、高校评测协作；status 为 participant-proposed / provisional / non-official；exit 为低于阈值、史实复核失败或出现隐私/文保/公共安全 P0 时 NO-GO，撤收或改动线。
- **T2 人才品质信号（北京AI原点社区）**：baseline 为试点前连续4周同一问卷、同一时段的匿名有效问卷；sample 为 baseline≥30、pilot≥30，目标有效率≥80%；window 为各4周；formula 为5点量表“导视是否支持停留/理解社区”的均值差 `pilot_mean-baseline_mean`；numeric threshold 为 `差值≥0.20分` 且有效率≥80%；owner 为社区议事/志愿者联盟牵头、高校评测协作；status 为 participant-proposed / provisional / non-official；exit 为低于阈值则不进入下一阶段，公开原因、重新设计并重新 baseline。
- **T3 运营供给（大钟寺AI产业聚集区／小月河场景赋能翼）**：baseline 为试点前连续4周匿名维护台账与能耗记录；sample 为 pilot 窗口不少于50条合格工单/服务请求；window 为8周，按连续2周滚动复核；formula 为 `24h内闭环工单/合格工单` 与 `误报工单/总告警工单`；numeric threshold 为24小时闭环率≥90%、误报率≤5%；owner 为大钟寺园区运营团队牵头、商户共治小组与辖区部门协作；status 为 participant-proposed / provisional / non-official；exit 为连续2周任一阈值不达、重识别风险或人工复核缺失则立即停项、下架并公示。
- **go/no-go 总闸**：baseline、sample、window 采用 T1–T3 各自协议，完成 T1 4周、T2 4周、T3 8周窗口后计算；formula 为 `通过协议数/3`；numeric threshold 为 `3/3` 且无隐私、文保、无障碍、交通安全 P0；owner 为参与主体组成的临时试点评议组，最终需专业团队确认；status 为 participant-proposed / provisional / non-official，不是现成验收承诺；exit 为任一协议不通过即 NO-GO，发布记录、撤收/改造，至少2周修正后重新 baseline 再议。

以上数值仅用于使测试可复现；当前不主张已具备场地、数据或许可条件，也不构成官方认可。启动前共同核定的版本、责任人和退出记录进入年度公示。
...
## 品牌识别与视觉系统（Logo/VI）
品牌识别固定为一套层级：中文主品牌「章痕·京张叙事导视带」；英文主品牌 **ZHANGHEN · JINGZHANG NARRATIVE WAYFINDING BELT**；描述性副标题为“文化叙事导视标识系统概念 / Cultural Storytelling Sign System Concept”；**WAYMARK·JZ 仅为内部工作代号**，不作为主标题、Logo 对外品牌或机构名称。图形以“道钉（点）—钢轨（线）—站牌（面）”三个母题组合，标准色为钢灰、信号红、道钉铜与主蓝；Logo/VI 基础见图。视觉系统规范（概念）：统一字体建议采用许可明确的思源黑体系列，统一图标线型与信息层级，全线图件、标识、数字界面与活动物料共用同一语汇。

品牌在先权利与使用边界：概念阶段未完成商标检索与法律审查，主品牌名称和图形仅作本方案评审展示，不对外注册、不商业使用、不作政府背书表示；WAYMARK·JZ 只作为内部工作代号。历史素材与第三方图库素材未经授权一律采用占位或公共领域替代物；正式使用前完成检索、法律审查与许可清点，来源与许可明细见 sources.json 资产条目。国际传播采用固定中英主品牌和描述性副标题，保证英文版图件、PDF与HTML与中文版逐项等价。
...
## 产业测试场景协议
三个产业测试场景用于验证"可读性"效应的三类可测试价值，测试结果仅作为概念迭代依据，不构成客流与投资承诺。三个场景分别对位三区（众智园、AI原点社区、大钟寺）的产业假设，下面的场景表与 `visual/assets/test_protocols.json` 采用同一套 T1/T2/T3 字段；详细协议明确 baseline、sample、window、formula、numeric threshold、owner、provisional/non-official 状态及失败退出。任何测试均不采集个体身份信息，问卷与台账匿名化处理。测试场景的选址、样本规模与运行时长在正式阶段由专业团队与相关主体共同核定，核定结果纳入年度公示；测试期间出现隐私风险或负面舆情超出阈值的，立即停止并公开说明。协议的具体执行由牵头运营主体负责、辖区部门与高校协作监督，测试台账按备案要求留存。

| 测试场景 | 测试目标 | 数据与许可边界 | 验收与退出 |
|---|---|---|---|
| T1 参访转化测试（众智园AI自主创新加速区／中关村科技服务翼） | 验证叙事导视对园区参访动线的正向引导 | baseline：试点前连续4周同一开放时段匿名预约台账；sample：两期各≥30个合格预约团组；window：baseline与pilot各4周、按星期配对；formula：`completed_visits / eligible_bookings`，`lift=(pilot_rate-baseline_rate)/baseline_rate`；baseline=0时只报绝对差 | numeric threshold：lift≥10%且两期sample均≥30；owner：园区公共空间运营方＋高校评测；状态：参与者提议、provisional、非官方；失败退出：低于阈值、史实复核失败或P0→NO-GO、撤收/调整动线 |
| T2 人才品质信号测试（北京AI原点社区） | 验证标识体验对人才驻留与理解社区的影响 | baseline：试点前连续4周同一匿名问卷、同一时段；sample：baseline≥30、pilot≥30份有效问卷，目标有效率≥80%；window：两期各4周；formula：5点量表条目均值差 `pilot_mean-baseline_mean` | numeric threshold：差值≥0.20分且有效率≥80%；owner：社区议事会／志愿者联盟＋高校评测；状态：参与者提议、provisional、非官方；失败退出：低于阈值→公开说明、重设计、重新采集baseline |
| T3 运营供给测试（大钟寺AI产业聚集区／小月河场景赋能翼） | 验证内容更新、维护与智能原生消费/商务场景的资源闭环 | baseline：试点前连续4周匿名维护台账与能耗记录；sample：pilot窗口≥50条合格工单/服务请求；window：8周、连续2周滚动复核；formula：`24h内闭环工单/合格工单`、`误报工单/总告警工单` | numeric threshold：24小时闭环率≥90%、误报率≤5%；owner：大钟寺园区运营团队＋商户共治小组＋辖区部门；状态：参与者提议、provisional、非官方；失败退出：任一阈值连续2周不达、重识别风险或人工复核缺失→立即停项、下架并公示 |
...
## 概念定位与复核声明
以上全部内容均为概念建议与参考方案：不含容积率、建筑高度、拆改留、投资预算、客流容量或工程可行性结论；不编造官方数据，不冒用官方背书；所有面积与比例为 provisional 低置信度取整值，官方数据发布后整体复算；「章痕·京张叙事导视带」与 ZHANGHEN · JINGZHANG NARRATIVE WAYFINDING BELT 是中英主品牌，WAYMARK·JZ 仅为内部工作代号，未完成在先权利检索前不对外注册使用；历史素材未授权前仅使用占位或公共领域替代物。中英双语版本逐条人工核对实质等价，任何差异以中文版为准并在修订时同步。
